...
#         print("Attempting to locate file for %s on %s: %s" % (symbol, trade_date, filename_string))
        ...
        filename_string = "vagrant_data/text_tick_data/%s" % (filename_string)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 6:
        if sys.argv[6] != "--output-deep-books-too":
            ...
            trades_output_file_name = "vagrant_data/book_snapshots/%s_trades.csv" % (trade_date)
            book_updates_output_file_name = "vagrant_data/book_snapshots/%s_book_updates.csv" % (trade_date)
